...
exports.MediaPicker.prototype.show = function(cb, max, type, message) {
	...
	var gotAssets = function(e) {
		...
		lastRow = false;
		assets = [];
		Ti.API.info('Items in Album:'+e.items.length);
		...
		Ti.API.info('Assets Length:'+assets.length);
		if (_env.iOS) {
			Ti.API.info('Env:IOS');
			Ti.API.info('Rows per screen:'+rowsPerScreen);
			var data = [];
	...
	var addRow = function(ret) {
		Ti.API.info('In Add row...'+rows.length);
		var toAdd = assets[rows.length];
		if (!toAdd) return false;
		Ti.API.info('Continuing to Add row...');
